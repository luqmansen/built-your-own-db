...
class BPlusTree:
    ...
    def graph(self, step, key=""):
        ...
        dot.render(f"graph.{step}", view=False)
    ...
    def merge_pdfs(self):
        ...
        pdfs = list([f for f in os.listdir() if f.endswith(".pdf")])

        merger = PdfWriter()

        for pdf in sorted(pdfs, key=lambda x: int(x.split(".")[1])):
            merger.append(pdf)

        merger.write("result.pdf")
        merger.close()
        # remove all pdfs
        for file in [f for f in os.listdir() if f.startswith("graph")]:
            try:
                os.remove(file)
            except Exception:
                continue
